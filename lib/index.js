...
function createDoc(options) {
  ...
  try {
    ...
    if (app.options.parse !== true) createOutputFiles(api);
...
function createOutputFiles(api) {
  if (app.options.simulate)
    app.log.warn(
      "!!! Simulation !!! No file or dir will be copied or created."
    );
  ...
  app.log.verbose(
    "copy template " + app.options.template + " to: " + app.options.dest
  );
  if (!app.options.simulate)
  ...
  if (!app.options.simulate)
    fs.writeFileSync(
      app.options.dest + "./api_project.json",
      api.project + "\n"
    );
  ...
  if (!app.options.simulate)
    fs.writeFileSync(
      app.options.dest + "./api_project.js",
      "define(" + api.project + ");" + "\n"
    );
}
